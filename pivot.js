...
(function() {
	...
	callWithJQuery(function($) {
		...
		pivotTableRenderer = function(pivotData, opts) {
			...
			for (i in rowKeys) {
				...
				for (j in rowKey) {
					...
					var sflag = false;
					for(var sx=0;sx<pivotData.numberFormatAttr.length;sx++){
						if(rowAttrs[j] == pivotData.numberFormatAttr[sx]){
							if(rowKey[j] && !isNaN(rowKey[j])){
								sflag = true;
								var sval = parseFloat(rowKey[j]).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,');
								txt = '<span style="text-align: right;display: block;">'+sval+'</span>';
							}
						}
					}
					for(var sx=0;sx<pivotData.percFormatAttr.length;sx++){
						if(rowAttrs[j] == pivotData.percFormatAttr[sx]){
							if(rowKey[j] && !isNaN(rowKey[j])){
								sflag = true;
								var sval = (parseFloat(rowKey[j])*100).toFixed(2)+"%";
								txt = '<span style="text-align: right;display: block;">'+sval+'</span>'
							}
						}
					}
		 ...
		 * 创建数据透视对象并在其上调用呈现器
		 * @param input 要操作的数据data
		 * @param inputOpts 自定义配置参数
		 * @param locale 本地化(en,zh)
		 * @returns 
		 **/
		 ...
		 * Pivot Table UI: 使用用户设置的选项调用上面的Pivot Table core
		 * @param input 要操作的数据data 是引用表的对象数组、数组数组、函数或jQuery对象
		 * @param inputOpts 自定义配置参数 
		 * @param overwrite 是一个布尔值，默认为false，用于控制在同一元素上重复调用此函数时发生的情况。如果设置为true，options对象将覆盖UI的当前状态。如果设置为false，则只有输入数据集更改，并且UI的状态保持不变，除非这是第一次调用，此时UI将被设置为选项。
		 * @param locale 本地化(en,zh) locale是一个默认为en的字符串，它控制数字格式和错误消息等内容的默认locale。不管此设置如何，您仍然可以覆盖默认聚合器（控制数字格式）和错误消息字符串。如果设置为en以外的其他值，则必须加载特定于区域设置的“语言包”，该语言包在调用pivotUI（）之前创建此区域设置对象。
		 * @returns 
		 **/
			...
			try {
				...
				refreshDelayed = (function(_this) {
					return function() {
						...
						subopts.numberFormatAttr = opts.numberFormatAttr;//sbw自定义
						subopts.percFormatAttr = opts.percFormatAttr;//sbw自定义
